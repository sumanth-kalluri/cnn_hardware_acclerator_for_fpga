...
import os
import math
from fixed_point_conversion import bin_to_float,float_to_bin,twos_comp

integer_precision = 7 
fraction_precision = 8
sign_bit = 0

NoT= 1                                                               #number of tests to run
klist = [3,5,7,11]                                                   #list of possible kernel sizes
...
for i in range(NoT):
	...
	fsize = np.random.randint(10,11,size = 1)
	...
	arr2 = np.random.uniform(-5,5,(ksize,ksize))
	arr = np.random.uniform(0,1,(fsize[0],fsize[0]))
	conv = strideConv(arr,arr2,1)
	for output in conv.flatten():
		num = float_to_bin(output,integer_precision,fraction_precision)
		file2.write(format(int(float_to_bin(output,integer_precision,fraction_precision),2),'08x')+"\n")
	file2.write("EoT"+str(i)+"\n")
	for x in arr2.flatten():
		file1.write(format(int(float_to_bin(x,integer_precision,fraction_precision),2),'04x')+"\n")
	for y in arr.flatten():
		file1.write(format(int(float_to_bin(y,integer_precision,fraction_precision),2),'04x')+"\n")
	file1.close()
	print(fsize)
	print(ksize)
	print(conv)
